...
function DetailPickTicket({ hideLoader }) {
    const { Id } = useParams();
    ...
    const Content = () => {
        ...
        const RejectTicket = () =>{
            AlertLoading()
            axios.post(global.Api+"RejectTicket",JSON.stringify({ReasonReject:ReasonReject,NikTechnician:localStorage.getItem('NIK'),IdTicket:Id}),{
                headers: {
                    'Content-Type': 'application/json',
                    'x-auth-token': localStorage.getItem("Token")
                }
                ...
                res =>{
                    setTimeout(
                        () => {
                            if (res.data.Status == "1") {
                                swal.close()
                                AlertSuccess('Success', 'Reject Ticket Is Success')
                                navigate('/ListPickTicket')
            ...
            <div>
                ...
                <div>
                    ...
                    <div className='row' style={{ marginTop: 100 }}>
                        <button style={{ marginRight: 20 }} className='btn btn-primary' onClick={SubmitResolution}>SUBMIT RESOLUTION</button>
                        <button style={{ marginRight: 20 }} data-toggle="modal" data-target="#myModal" className='btn btn-success'> <i className="fas fa-chalkboard-teacher"></i> ASSIGN</button>
                        <button data-toggle="modal" data-target="#FormRejectTicket" className='btn btn-danger'> <i className="fas fa-trash"></i> REJECT</button>
                ...
                <div className="modal" id="FormRejectTicket">
                    <div className="modal-dialog">
                        <div className="modal-content">


                            <div className="modal-header">
                                <h4 className="modal-title">Reject Ticket</h4>
                            ...
                            <div className="modal-body">
                                ...
                                <textarea className='form-control' required onChange={(e) => setReasonReject(e.target.value)}></textarea>
                                <br/>
                                <button className='btn btn-primary' data-dismiss="modal"
                                onClick={()=>{
                                    if (ReasonReject == "") {
                                        AlertError("Error","Reason Reject Is Required")
                                    } else {
                                        RejectTicket()
                                    }
                                }}
...
export default DetailPickTicket